...
import { fetchBestSellingProducts } from '@/lib/products'
...
export const BestSellingProducts: React.FC = () => {
  ...
  useEffect(() => {
    const loadBestSellingProducts = async () => {
      try {
        setLoading(true)
        setError(null)
        const bestSellingProducts = await fetchBestSellingProducts(6)
  ...
  if (error || products.length === 0) {
    return null // Don't show section if no best selling products
  }
